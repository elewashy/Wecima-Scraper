...
def scrape(target_url):
    full_url = f"https://wecima.video/watch/{target_url}"

    headers = {
        'Referer': 'https://wecima.video/',
        ...
        if response.status_code == 200:
            ...
            if episodes_container:
                ...
                for episode in episodes:
                    episode_url = episode['href'].replace("https://wecima.video", "")  # إزالة الدومين
        ...
        if response.status_code == 200:
            ...
            if download_section:
                ...
                if episodes_container:
                    episodes = episodes_container.find_all('a', class_='hoverable activable')
                    for episode in episodes:
                        episode_url = episode['href'].replace("https://wecima.video", "").replace("/watch", "/download")  # تعديل الرابط ليبدأ بـ /download
